...
Этот каталог — про живую доставку собранных сетей по BGP на **bare-metal/systemd**. Сами списки
генерирует `bgpcollect collect` (см. корневой README), а здесь — как анонсировать их подписчикам.

> Хотите всё в Docker? Готовый контейнер с настроенным BIRD есть в compose: профили `bgp` /
> `bgp-demo` (см. корневой README, раздел «Живой BGP-фид»). Там используется режим
> `--route-dest blackhole` + `next hop self` — он надёжно работает внутри контейнера, где
> next-hop недостижим. Ниже — вариант для запуска BIRD прямо на хосте.
